...
const MessageContainer = () => {
  ...
  const {selectedChatType, selectedChatData,userInfo,setSelectedChatMessages, selectedChatMessages,setFileDownloadProgress,setIsDownloading} =useAppStore()
  ...
  const renderDMMessages = (message)=>(
    <div className={`${message.sender === selectedChatData._id ? "text-left" : "text-right" }`}>
    {
      message.messageType==="text" && (
        <div className={`${message.sender !== selectedChatData._id ? "bg-[#8417ff]/5 text-[#8417ff]/90 border-[#V8417ff]/50 ":"bg-[#2a2b33]/5 text-white/80 border-[#ffffff]/20"} border inline-block p-4 rounded my-1 max-w-[50%] break-words `} >
        {message.content}
        </div>
      )}
      {
  ...
  <div className="cursor-pointer" onClick={()=>{
    ...
    <img src={`${HOST}/${message.fileUrl}`} width={200} height={200} />
  </div>
) : 
  ...
  const renderChannelMessages=(message)=>{
    return (
      ...
      >
        {message.messageType==="text" && (
          <div 
            className={`${
              ...
              ? "bg-[#8417ff]/5 text-[#8417ff]/90 border-[#V8417ff]/50 "
        ...
        {
        message.messageType==="file" && (
        <div className={`${message.sender._id === userInfo._id ? "bg-[#8417ff]/5 text-[#8417ff]/90 border-[#V8417ff]/50 ":"bg-[#2a2b33]/5 text-white/80 border-[#ffffff]/20"} border inline-block p-4 rounded my-1 max-w-[50%] break-words `} >
        {checkIfImage(message.fileUrl) ? (
        <div className="cursor-pointer" onClick={()=>{
        setShowImage(true)
       setImageURL(message.fileUrl)
          ...
          message.sender._id !== userInfo.id ? <div className="flex items-center justify-start gap-3 ">
            <Avatar className="h-8 w-8 rounded-full overflow-hidden">
                  ...
                  message.sender.image &&
                   (
                    <AvatarImage src={`${HOST}/${message.sender.image}`} alt="profile"className="object-cover rounded-full w-full h-full bg-black"/>
                   )}
