        ...
        <div className="grid grid-cols-2">
          ...
          <div className="flex min-w-32 md:min-w-64 flex-col gap-5 text-white items-center justify-center">
            ...
            <div className="w-full flex gap-5 ">
              {
                colors.map((color,index)=>(<div className={`${color} h-8 w-8 cursor-pointer transition-all duration-300 rounded-full ${selectedColor===index ?"outline outline-white/50 outline-1":""}}`} key={index} onClick={()=>setSelectedColor(index)}></div>
              ))}
            </div>
          </div>
        </div>
        <div className="w-full">
          <Button className="bg-purple-700 hover:bg-purple-900 transition-all duration-300 h-16 w-full" onClick={saveChanges}>
            Save Changes
          </Button>
        </div>
      </div>
